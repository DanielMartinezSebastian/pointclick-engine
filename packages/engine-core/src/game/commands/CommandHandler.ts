...
export class CommandHandler {
  ...
  register<T extends GameCommandType>(type: T, executor: Executor<T>): () => void {
    (this.executors as Record<string, unknown>)[type] = executor;
    return () => {
  ...
  execute(cmd: GameCommand): void {
    const executor = (
      this.executors as Record<string, Executor<GameCommandType> | undefined>
    )[cmd.type];
    if (!executor) {
      this.unknownLogger(cmd);
      return;
    }
    (executor as (cmd: GameCommand) => void)(cmd);
  }
